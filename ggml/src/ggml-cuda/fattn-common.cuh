...
static inline bool ggml_cuda_fattn_band_wmma_applies(const int cc, const ggml_tensor * dst) {
    ...
    // reported by @DanoPTT).  bf16 reaches the band through its opt-in native arm (kv_native = BF16)
    // and is deliberately NOT banded while it is staged, where the whole-cache conversion dominates.
